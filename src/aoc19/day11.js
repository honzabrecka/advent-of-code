...
const paint = (m) => {
  ...
  const getColor = () => m[hash(position)] || 0
  const setColor = (color) => m[hash(position)] = color

  while (true) {
    const [op, value] = runUntilIO(gen)
    if (op == io.END) break
    if (op == io.INPUT) write(getColor())
    if (op == io.OUTPUT) {
      setColor(value)
      const [, turn] = runUntilIO(gen)
      position = move(position, rotate(turn))
    }
  }
...
const main2 = () => {
  ...
  const max = (col) => col.sort((a, b) => b - a)[0]
  const p = { 0: ' ', 1: 'X' }

  for (let y = 0; y < max(ys) + 1; y++) {
    let row = ''
    for (let x = 0; x < max(xs) + 1; x++) {
      row += p[m[hash([x, y])] || 0]
    }
    console.log(row)
  }

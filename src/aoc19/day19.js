...
const getValue = (x, y) => {
  ...
  runUntilIO(gen)
  write(y)
  const [, response] = runUntilIO(gen)
  return response
}

const occupied = (x, y) => getValue(x, y) == 1
    ...
    for (let x = 0; x < 5000; x++) {
      ...
      if (occupied(x, y)) {
        if (
          ...
          occupied(x, y - size) &&
          // top-right
          occupied(x + size, y - size) &&
          // bottom-right
          occupied(x + size, y)
        ) {
          console.log(x, y - size, 10000 * x + (y - size))
          process.exit(0)
        } else {
          break;
        }
